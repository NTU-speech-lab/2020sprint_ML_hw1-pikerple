...
for t in range(iter_time):
    ...
    train_loss_9.append(t_loss)
    dev_loss_9.append(d_loss)
    if(t%100==0):
...
x = np.empty([12 * 471, 1 * num], dtype = float)
y = np.empty([12 * 471, 1], dtype = float)
for month in range(12):
    for day in range(20):
        for hour in range(24):
            if day == 19 and hour > 14:
                continue
            x[month * 471 + day * 24 + hour, :] = month_data[month][9,day * 24 + hour : day * 24 + hour + num].reshape(1, -1) #vector dim:18*9 (9 9 9 9 9 9 9 9 9 9 9 9 9 9 9 9 9 9)
...
for t in range(iter_time):
    t_loss = np.sqrt(np.sum(np.power(np.dot(x, w) - y, 2))/471/12)#rmse
    d_loss = np.sqrt(np.sum(np.power(np.dot(x_validation, w) - y_validation, 2))/1131)
    train_loss_5.append(t_loss)
    dev_loss_5.append(d_loss)
    if(t%100==0):
        print(str(t) + ":" + str(t_loss))
        print(str(t) + ":" + str(d_loss))
    gradient = 2 * np.dot(x.transpose(), np.dot(x, w) - y) #dim*1
...
# with open('output.csv', mode='w', newline='') as submit_file:
...
plt.plot(dev_loss_5)
plt.title('Loss')
plt.legend(['train_9','train_5','dev_9','dev_5'])
plt.savefig('loss_9_5.png')
plt.show()        #print(row)
